...
//            MethodUtils.invokeMethod(POLICY_BUILD_OBJ, "withInterval", 1);
    ...
    // s -> server logger
    // h -> history logger
    private static Map<String, com.jpda.jpdademo.log.InternalLogger> loggerMap = new HashMap<>();

    static {
        try {
            com.jpda.jpdademo.log.InternalLogger logger;
//            // server logger
...
//            loggerMap.put("h", logger);
     ...
     * @return a logger
     */
    public static com.jpda.jpdademo.log.InternalLogger getLogger(String s) {
        com.jpda.jpdademo.log.InternalLogger logger = loggerMap.get(s);
        if (logger == null) {
